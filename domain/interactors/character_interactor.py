...
async def import_character(url, user_id, guild_id) -> Character:
    try:
...
async def update_character(user_id, guild_id) -> Character:
    character = config.repository.get(user_id, guild_id)
            
    if character:
        return await import_character(f'{config.DEMIPLANE_URL}{character.character_id}', str(user_id), str(guild_id))
    else:
        raise ReferenceError("No character recovored")
    
def get_character(user_id, guild_id) -> Character:
    character = config.repository.get(user_id, guild_id)
    if not character:
        raise exceptions.CharacterImportError()
...
def get_character_list(user_id, guild_id) -> List[Character]:
    return config.repository.get_all(user_id, guild_id)

def select_character(user_id, guild_id, character_id) -> Character:
    character = config.repository.set_primary(user_id, guild_id, character_id)
    if not character:
        raise exceptions.CharacterImportError()
